resource "aws_ecs_cluster" "ecs-web-cluster" {
...
resource "aws_ecs_task_definition" "task-definition-test" {
  family                = "web-family"
  container_definitions = file("templates/container-def.json")
  network_mode          = "bridge"
  tags = {
    "enviroment" = var.environment
    "createdBy"  = var.owner-resource
...
resource "aws_ecs_service" "service" {
  ...
  cluster         = aws_ecs_cluster.ecs-web-cluster.id
